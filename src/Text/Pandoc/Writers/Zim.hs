...
{- |
   ...
   Copyright   : Copyright (C) 2014 Allan Gardner <mathnerd314.gph+pdc@gmail.com>
...
Note that Zim is relatively limited in terms of syntax; for example, there are no escape characters.
pandoc will insert spaces whenever an escape character would be used.
Also, footnotes and tables are not possible in Zim.
If pandoc encounters one of these, it will transliterate it (e.g., tables will be written as CSV).
...
-- escapeStringWithTextEnding :: String -> String
...
blockToZim HorizontalRule = return $ blankline <> text "----------------" <> blankline -- not implemented (https://bugs.launchpad.net/zim/+bug/514945)
...
{-
citeArguments :: [Inline] -> [Inline] -> String -> WS Doc
...
-- external link:
-- Handle Zim-like internal document references to sections
-- or my@email.com[email john]
-- relative:  link:downloads/foo.zip[download foo.zip]
inlineToZim (Link [Image alt (imgsrc,imgtit)] (src, tit)) = do
  undefined
inlineToZim (Link elements@txt (url@src, title@tit)) = do
--  case txt of [Str s] | escapeURI s == src -> True -- autolink
--  text $ " \"" <> tit <> "\""
  let src' = escapeURI src
  contents <- inlinesToZim txt -- escape somehow
  -- mailto link?
